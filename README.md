...
A **measurement** is a named collection of one or more **data series**. For example, a measurement called `weather_station` might contain series `temperature`, `humidity`, and `pressure`. All data series within a measurement share a common x-axis — typically a list of timestamps — with each series providing a corresponding value at each point. Plain numeric x-values are also supported for non-time data.
...
```json
{
  ...
  "points": {
    ...
    "series": {
      "value":    [42.1, 43.5, 41.8],
      "smoothed": [42.0, 42.8, 42.2]
    }
  }
}
```
...
ingest({
  ...
  time: false,
  points: { timestamps: [0, 1, 2], series: { value: [1, 2, 3] } }
});
```
...
| Name                                                                    | Description                                                                                          |
| ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `ingest({ measurementName, points: { timestamps, series: {...} } })`    | Writes data into the store. `series` is `{ seriesName: values[] }`. Supports `clearMeasurement` and `time` flags. |
...
Available globals: `Math`, `JSON`, `Date`, `Array`, `Object`, `Map`, `Set`, `String`, `Number`, `Boolean`, `Error`, `NaN`, `Infinity`, `parseFloat`, `parseInt`, `isFinite`, `isNaN`. DOM, `fetch`, and filesystem access are **not** available.
...
| `median-processor` | Applies a sliding-window median filter to a source series. Effective at removing impulse noise (spikes) while preserving edges. Configurable window size, source and output series names. |
...
| `ingest({ measurementName, points: { timestamps, series: {...} } })`    | Writes derived data. `series` is `{ seriesName: values[] }`. Non-destructive merge (sibling series untouched). Supports `clearMeasurement` and `time` flags. |
| `require(moduleName)`                                                   | Load a built-in library. Currently: `'dsp'`.                                                                                                    |
...
| `dsp.median(window)`  | Stateful sliding-window median filter. `window` is the number of samples (integer ≥ 1; odd values conventional). Methods: `update(values[])` → filtered `number[]`, `reset()`.                                                                                                                                                     |
